...
class ImageSearchEngine:
    ...
    def _extract_entities(
        self, fact_text: str, place_name: str
    ) -> dict[str, list[str]]:
        """Extract relevant entities from fact text."""
        entities = {"people": [], "buildings": [], "streets": [], "landmarks": []}
        ...
        entities["people"] = [
            name for name in potential_names if not any(nn in name for nn in non_names)
        ][
            :3
        ]  # Limit to 3 most likely names
        ...
        for keyword in building_keywords:
            if keyword in fact_lower:
                ...
                for match in matches:
                    ...
                    if len(building_name) > len(
                        keyword
                    ):  # Only if we found more than just the keyword
    ...
    async def _search_building_images(
        self, building_name: str, coordinates: tuple[float, float]
    ) -> list[str]:
        ...
        try:
            ...
            for item in data.get("query", {}).get("search", []):
                ...
                if title.startswith("File:"):
                    ...
                    if any(
                        skip in title_lower
                        for skip in ["logo", "map", "plan", "carte", "схема"]
                    ):
                        continue
    ...
    async def _search_place_images(
        self, place_name: str, coordinates: tuple[float, float]
    ) -> list[str]:
    ...
    async def _geosearch_commons(
        self, coordinates: tuple[float, float], radius: int = 200
    ) -> list[str]:
    ...
    async def _get_wikidata_image(self, qid: str) -> str | None:
    ...
    async def _get_image_url(self, file_title: str, width: int = 1600) -> str | None:
    ...
    def _rank_images(
        self, images: list[str], fact_text: str, place_name: str, max_count: int
    ) -> list[str]:
