...
class OpenAIClient:
    ...
    async def get_nearby_fact(
        ...
        try:
            ...
            previous_facts_text = ""
            if previous_facts:
                # Extract already mentioned locations to explicitly ban them
                mentioned_locations = []
                for fact in previous_facts[-5:]:
                    # Extract location name from format "Location: fact text"
                    if ":" in fact:
                ...
                previous_facts_text = (
                    ...
                    + "\n".join([f"- {loc} (уже было!)" for loc in mentioned_locations])
                    + "\n\n🎯 ОБЯЗАТЕЛЬНО найдите ДРУГОЕ место в радиусе 200-500м:\n"
                    + "• Другие улицы, площади, здания, памятники\n"
                    + "• Другие парки, сады, достопримечательности\n" 
                    + "• Архитектурные объекты, музеи, церкви\n"
                    + "• ЛЮБОЕ место, НО НЕ из запрещенного списка выше!\n\n"
                    + "РАНЕЕ РАССКАЗАННЫЕ ФАКТЫ (для справки):\n"
            ...
            if is_live_location:
                ...
                user_prompt = (
                    ...
                    + ("   ⚠️ КРИТИЧЕСКИ ВАЖНО: НЕ ПОВТОРЯЙТЕ места из запрещенного списка!\n" if previous_facts else "")
                    ...
                    + ("🚨 ОБЯЗАТЕЛЬНОЕ ТРЕБОВАНИЕ: Найдите ДРУГОЕ место, НЕ из списка выше!\n\n" if previous_facts else "")
                    + "🎯 СТРАТЕГИЯ ПОИСКА" + (" (НОВОЕ место!):" if previous_facts else ":") + "\n"
                    "• " + ("СТРОГО ИЗБЕГАЙТЕ запрещенных мест — найдите альтернативу в том же районе\n• " if previous_facts else "")
